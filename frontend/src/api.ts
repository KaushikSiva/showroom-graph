export interface Product {id:string;name:string;brand?:string;category:string;price:number;currency?:string;image_url?:string;source_url:string;dimensions?:string|Record<string,unknown>;reason?:string;rationale?:string;}
export interface Directive {id:string;instruction:string;prompt:string;status:string;created_at:string;message?:string;}
export interface Room {id:string;name:string;budget:number;preferences:string[];keep:string[];image_url:string|null;directives:Directive[];products:Product[];total:number;graph:{status:string;explanation?:string;[key:string]:unknown};exports:{id?:string;url?:string;title?:string;type?:string;[key:string]:unknown}[];}
